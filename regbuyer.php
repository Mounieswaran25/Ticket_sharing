...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO buyer_user(bname, email, mol, pswd, ads) 
            VALUES ('$name', '$email', '$mobile', '$password', '$address')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('Registered Successfully!'); window.location.href='buyerlogin.php';</script>";
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>User Registration - Ticket Sharing</title>
    ...
    <style>
        body {
            margin: 0;
            padding: 0;
            background: url('images/bus4.jpg') no-repeat center center fixed;
        ...
        <form action="" method="POST">
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="bname" id="uname" class="form-control" required>
                </div>
                <div class="col-md-6">
                ...
                <p class="register-text text-white">Already have an account? <a href="buyerlogin.php" class="register-link">Login</a></p>
            </div>
            </div>
        </form>
    </div>
